...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        //calculate answers according to user selections
        val viewHistoryButton: Button = findViewById(R.id.history_button)
    ...
    //update next question and update marks
    private fun startGame() {
        option1Button.setBackgroundResource(R.drawable.rounded_background_box)
        option2Button.setBackgroundResource(R.drawable.rounded_background_box)
        option3Button.setBackgroundResource(R.drawable.rounded_background_box)
        option4Button.setBackgroundResource(R.drawable.rounded_background_box)
    ...
    private fun showNextQuestion() {
        ...
        updateQuestionUI()
        startTimer()
        questionNumberTextView.text = "    Question $currentQuestionNumber   "
    ...
    //timer start when the question load
    ...
    private fun endGame() {
        // Display final score in toast message
        Toast.makeText(this, "Game Over! Your score: $score", Toast.LENGTH_SHORT).show()

        // Save the score to history in shared preferences
        val sharedPreferences = getSharedPreferences("MyPrefs", Context.MODE_PRIVATE)
        val history = loadHistory() // Load existing history
        val newHistory: String
        if (history.isEmpty()) {
            newHistory = "Score 1: $score" // If no history exists, start with Score 1
        } else {
            newHistory = "Score ${history.split("\n").size + 1}: $score\n$history" // Append new score to existing history
        }
        sharedPreferences.edit().putString("history", newHistory).apply()

        // Hide game layout and show dashboard
    ...
    fun onOptionSelected(view: View) {
        ...
        Handler(Looper.getMainLooper()).postDelayed({
            view.setBackgroundResource(R.drawable.rounded_background_box)
